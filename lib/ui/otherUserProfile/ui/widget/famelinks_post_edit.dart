...
class FameLinksPostEditScreen extends StatefulWidget {
  const FameLinksPostEditScreen(
      {Key? key,
      required this.postData,
      required this.index,
      required this.provider})
      : super(key: key);
  final oldData.GetParticularUserProfileModelResult postData;
  final int index;
  final GetParticularFameLinksProfileProvider provider;

  @override
  State<FameLinksPostEditScreen> createState() =>
      _FameLinksPostEditScreenState();
}

class _FameLinksPostEditScreenState extends State<FameLinksPostEditScreen> {
  ...
  void updatePost() {
    ...
    Api.put.call(context,
        method: 'media/famelinks/${widget.postData.sId}',
                ...
                child: PageView.builder(
                    ...
                    itemBuilder: (context, int index) {
                      return widget.postData.media![index].type != 'video'
                          ? CachedNetworkImage(
                              imageUrl:
                                  '${ApiProvider.s3UrlPath}/${ApiProvider.famelinks}/${widget.postData.media![index].path}',
                            )
                          : Stack(fit: StackFit.expand, children: [
                              CachedNetworkImage(
                                  imageUrl:
                                      '${ApiProvider.s3UrlPath}/${ApiProvider.famelinks}/${widget.postData.media![index].path}-xs'),
                              Align(
