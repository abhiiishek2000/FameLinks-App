...
class FunLinksPostEditScreen extends StatefulWidget {
  const FunLinksPostEditScreen(
      {Key? key,
  ...
  final int index;
  final GetParticularFunLinksProfileProvider provider;

  @override
  State<FunLinksPostEditScreen> createState() => _FunLinksPostEditScreenState();
}

class _FunLinksPostEditScreenState extends State<FunLinksPostEditScreen> {
  ...
  void updatePost() {
    ...
    Api.put.call(context,
        method: 'media/funlinks/${widget.postData.sId}',
                ...
                child: PageView.builder(
                    ...
                    itemBuilder: (context, int index) {
                      return widget.postData.media![index].type != 'video'
                          ? CachedNetworkImage(
                              imageUrl:
                                  '${ApiProvider.s3UrlPath}/${ApiProvider.funlinks}/${widget.postData.media![index].path}',
                            )
                          : Stack(fit: StackFit.expand, children: [
                              CachedNetworkImage(
                                  imageUrl:
                                      '${ApiProvider.s3UrlPath}/${ApiProvider.funlinks}/${widget.postData.media![index].path}-xs'),
                              Align(
