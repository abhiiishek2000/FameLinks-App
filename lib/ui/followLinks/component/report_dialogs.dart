...
import '../../../models/InfoPopupModel.dart';
  ...
  if (result != null) {
    if (result == true) {
      ...
      if (result != null) {
        ...
        MediaCompressionProvider().compressVideo(context, result,
            onSave: (String? outputPath) async {
          Constants.video = await MultipartFile.fromFile(outputPath!,
...
void showReportPostDialog(
    ...
    FollowLinksFeedProvider provider,
    String userId,
    String postId,
...
showBlockAlertDialog(BuildContext context, String userId) {
  var provider =
...
showRestrictAlertDialog(BuildContext context, String userId) {
  final pro = Provider.of<FollowLinksFeedProvider>(context, listen: false);
  // set up the button
  Widget okButton = TextButton(
    child: Text("Yes", style: GoogleFonts.nunitoSans(color: lightRed)),
    onPressed: () async {
...
void showFollowLinkShareDialog(BuildContext context, String mediapath,
    String mediatype, int index, int ind) {
  showDialog(
      context: context,
      builder: (BuildContext context) {
        return Consumer<FollowLinksFeedProvider>(
            builder: (context, dataFollowLink, child) {
          return Dialog(
            elevation: 0,
            backgroundColor: Colors.transparent,
            insetPadding: EdgeInsets.only(
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    InkWell(
                      onTap: () {
                        var data = Provider.of<FollowLinksFeedProvider>(context,
                                listen: false)
                            .followLinkList[index];
                    ...
                    InkWell(
                      onTap: () async {
                        Navigator.pop(context2);
                        Sharefiles().onShareFollow(
                            mediapath, mediatype, index, ind, context);
                      },
                      child: Padding(
                          padding: EdgeInsets.only(
                              top: ScreenUtil().setSp(12),
