...
class RepliesViewFunLinks extends StatefulWidget {
  RepliesViewFunLinks({Key? key,required this.postId,required this.userId,required this.newUserId,required this.type,required this.index,required this.comment}) : super(key: key);
  Comment comment;
  int index;
  String type;
  String postId;
  ...
  @override
  State<RepliesViewFunLinks> createState() => _RepliesViewFunLinksState();
}

class _RepliesViewFunLinksState extends State<RepliesViewFunLinks> {
  ...
  void initState() {
    commentRepliesScrollController.addListener(() {
      if(commentRepliesScrollController.position.maxScrollExtent == commentRepliesScrollController.offset){
        Provider.of<FunLinksFeedProvider>(context,listen: false).getCommentReplies(context, widget.comment.sId!,isPaginate: true);
      }
    });
    super.initState();
  }
